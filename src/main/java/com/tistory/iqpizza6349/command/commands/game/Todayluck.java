...
public class Todayluck implements ICommand {
    ...
    public void handle(CommandContext ctx) {
        ...
        String s = getisCard(user.getIdLong());
        String check = null;
        if(s != null){
            if(!s.equals("null")){
                check = s.split("-")[0];
        ...
        StringBuilder sbb = new StringBuilder(to);
        sbb.append(decimalFormat.format(calendar.get(Calendar.MONTH)));
        sbb.append(decimalFormat.format(calendar.get(Calendar.YEAR)));
        String todate = sbb.toString();
        ...
        if (card.equals(lucky.get(1))) {
            ...
            channel.sendMessage("\n운세 풀이 : 오늘은 무언가 시작이 될 것 같군요 하지만 조금 불안정한 시작이에요. " +
                    "\n예를들어 학교에서 급히 새로운 과제를 내 줄수도 있을것 같아요 또 대학 입학일 수도 있고요. 그리고 오늘은 행동과 말을 조금더 생각해본뒤 실행해보는건 어떨까요?").queue();
            updateToday(card,todate,user.getIdLong());
        } else if (card.equals(lucky.get(2))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 이 카드는 재능, 창조라는 뜻이 있어요, 하지만 겁많음, 기만 이라는 뜻도 있죠.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘 시작하는 일이 아주 잘될거에요, 또 새로운 계획이 성공할 것이고요. 오늘은 상상력과 자신의 아이디어를 살리는게 좋아보여요. " +
                    "\n하지만 오늘 사기를 당할 수 도 있으니 조심하셔야 되요.").queue();
            updateToday(card,todate,user.getIdLong());

        } else if (card.equals(lucky.get(3))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 지식, 지혜를 뜻합니다. 반대로 무지, 무례함을 뜻하기도 하죠").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 당신의 지식이 빛을 발할 것 같아요. 또 침착함을 잃지 않을 것 같아요. " +
                    "\n하지만 조금 외로워 보이기도 해요. 오늘 자신의 생각 또는 고민에 너무 집착하지는 마세요. 그게 힘들다면 잠시 바람을 쐬는건 어떨까요?").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(4))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 :풍요 모성을 뜻하고,동요 태만 과잉을 뜻합니다. ").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 여성스러움이 강한것 같아요. 혹은 예술적 재능, 포용력이 폭발하는 날이에요. 주목을 잘 받을 수도 있겠어요. " +
                    "\n오늘은 동요하지말고 태만하지 않으면 좋을것 같아요.").queue();
            updateToday(card,todate,user.getIdLong());
            ...
            channel.sendMessage("\n운세 풀이 : 오늘은 의지력이 빛을 발해 오늘 할일을 잘 끝 마칠것 같아요. 오늘은 체력이 충분해 쌩썡 할 수도 있고요, 남이 당신에게 더욱 기댈 수 있을것 같아요." +
                    "\n하지만 업무과다나 실무 능력이 부족하면 무리해서 할 필요가 없어요. 허세를 부리지 않는게 좋아요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(6))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 가르침 관대함을 뜻하고, 나태 신용이 떨어짐을 의미하기도 해요").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 솔직한 조언을 받을 것 같아요. 또한 당신이 다른사람을 이끌어 갈 수 있을 거에요." +
                    "\n하지만 너무 조언에만 메어있으면 좋지않을 수 있어요. 또 누군가에게 신용을 잃을 수 있고요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(7))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 연에 결합이라는 뜻이 있고, 배신, 실연이라는 뜻이 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 사랑이 이루어 질 수 있을 것 같아요. 또한 인연이 찾아 올 수도 있고요. 협력자나 파트너가 좋은 사람일 경우가 많아요." +
                    "\n하지만 갑자기 실연을 맞을 수 있고, 혼란스러운 일이 있을 수 있어요. 그리고 배신을 당할 수 도 있으니 조심하는게 좋아요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(8))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 전진 승리라는 뜻이 있고, 좌절 패배라는 뜻이 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 뭔가 이길 일이 있을것 같네요. 오늘은 하나의 전차가 된거같이 적극적으로 행동하는것이 좋을 것 같아요." +
                    "\n하지만 너무 성급하면 오히려 독이 될 수 있으니 조심하는게 좋아요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(9))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 의지 용기의 뜻이 있고, 무기력 자만이라는 뜻이 있어요").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 이제까지 해결하지 못한 난관을 의지력으로 해결할 수 있을 것같아요." +
                    "\n하지만 오늘 무기력 하거나 인내심이 부족하다면 쉬는것도 하나의 방법이에요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(10))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 탐구, 사려깊은이라는 뜻이 있고, 폐쇄적 음습이라는 뜻이 있어요").queue();
            channel.sendMessage("\n운세 풀이 : 오늘 하는 모든일에 신중함이 묻어 나오는것 같아요. 조용히 묵묵히 할일을 하는것 같기도 하고요." +
                    "\n하지만 쓸쓸해 보이기도해요. 오늘은 비밀을 누설하거나 침묵을 깨는일이 없도록 조심하는게 좋아요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(11))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 윤회 일시적인 행운이라는 뜻이 있고, 오산 불운의 뜻이 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 전환기를 맞을 것 같아요. 생각지 못한행운이 찾아올 수 도있고요." +
                    "\n 하지만 오늘은 예상에 빗겨나가는 일이 있을 수 있어요. 불리한 입장이 될 수 도 있으니 조심하는게 좋아요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(12))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 균형 정당함을 뜻하고, 편견 부정이라는 뜻이 있어요.\n").queue();
            channel.sendMessage("운세 풀이 : 오늘은 당신이 조정하는 역할을 할 것 같아요. 중립적인 입장을 지키는것이 좋아 보여요." +
            ...
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 격변 끝이라는 뜻이 있고, 변화에 저항 앞으로 나아갈수 없음이라는 뜻도 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 무언가 크게 변화할 것 같아요. 변화를 위해 이때까지 이루어온 것을 포기해야 할 수도 있어요." +
                    "\n하지만 손해를 보더라도 나아가기 위해 버리는 것이 나아 보여요. 용기있게 변화에 직면하는것이 좋아보여요. " +
                    "\n카드뜻 그대로 누군가가 죽을 수 도 있으니 마음을 단단히 먹는것이 좋아요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(15))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 조화와 스스로 자제라는 뜻이 있고, 낭비 극단적인 이란 뜻이 있어요.").queue();
            channel.sendMessage("운세 풀이 : 오늘은 당신의 생각을 한번 더 생각해보고 말하는 것이 좋을 것 같아요. 열린자세로 상대방을 받아들일 준비를 해 두는것도 좋을 것 같아요." +
                    "\n오늘은 당신을 절제하며 욕망을 조절하는 것이 중요해요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(16))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 사심 의존이라는 뜻이있고, 안좋은 상황으로부터 회복 떨쳐내고 자유로워짐 이라는 뜻이 있어요. ").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 당신이 만들어낸 상황에 억압되어 있을 수 도 있어요. 과하게 약속이 잡혀 있다면 조금은 취소하는 것이 좋아요." +
                    "\n또 당신이 두려움과 부정적인 생각이 있다면 그것을 직면해 포용하는것이 중요해요. 힘들겠지만 포용한뒤에는 한껏 자유로워질거에요.").queue();

            updateToday(card,todate,user.getIdLong());
        }else if (card.equals(lucky.get(17))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 재난 파멸을 뜻하고, 변화에 대한 두려움 재앙을 회피라는 뜻이 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 무언가 극적으로 바뀔 것 같아요. 바뀔것을 지키는것 보다 빠르게 포기하고 새롭개 무언가를 다시 구축하는게 좋아요." +
                    "\n갑자기 무언가 변하고, 잃고, 무너지기 때문에 마음을 단단히 먹는게 좋아요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(18))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 희망 평화라는 뜻이있고, 절망 환멸이라는 뜻이 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 희망찰 거에요. 당신의 판단을 믿는 것이 좋아요. 오늘은 당신이 이루고 싶었던것을 이룰 수 있을 거에요." +
                    "\n하지만 자신감이 부족하거나 비관적인 태도를 취하면 당신이 즐기는 것을 방해할 수 있어요. 되도록 부정적인 태도보다 긍정적인 태도를 취해보는것은 어떨까요?").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(19))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 불안 자기기만이란 뜻이있고, 혼돈의 끝 모든것을 해결 이란 뜻이 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 뭔가 불안한 감정과 혼란을 가지고 있을 것 같아요. 이러한 감정을 가지게 하는것에 대해 알아내는것이 중요해요. 또한 말하지 못한것과 비밀스러운 것들이 밝혀질 것이에요." +
                    "\n이 혼란이 끝날때까지 결론을 내지 않는것이 좋아요. 오늘은 당신의 직관을 신뢰해 보는것도 좋아요.").queue();

            updateToday(card,todate,user.getIdLong());
        }else if (card.equals(lucky.get(20))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 밝은미래 성공이란 뜻과, 일시적인 우울감 실패라는 뜻이 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘 하는 일이 다 성공할 것 같아요. 또 활력이 넘치고 건간할 거에요." +
                    "\n하지만 당신이 가진 목표가 비현실적인 것이라면 성공하지 못할 수 도 있어요. 또 원하는 것을 얻어도 결과가 마음에 안들 수 도 있고요." +
                    "\n자신의 성공에 오만하게 대하지 않는 것이 중요해요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(21))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 심판 부활이란 뜻과, 후회 확신이 없는이란 뜻이 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 새로운 발전에 대해 준비를 해야할 것 같아요. 당신의 인생계획을 바꿀 중요한 결정을 만날 수 도 있고요. 변화를 선택할때 잠재력을 알아 차린다면 그 결과는 유익할 거에요." +
                    "\n또 당신에게 모든일이 유리하게 적용될 것 같아요." +
                    "\n하지만 잘못된 선택을 하지 않는 것이 중요해요. 중요한 결정을 피하지 않고, 진실을 마주하는것이 힘들지라도 마주하고 과거를 버리는 것이 좋아요.").queue();
            updateToday(card,todate,user.getIdLong());

        }else if (card.equals(lucky.get(22))) {
            channel.sendMessage(user.getName() + "님의 오늘의 운세는 " + card + " 입니다.\n").queue();
            channel.sendMessage("카드 뜻 : 완성 완전한 이라는 뜻과, 미완성 아직 끝나지 않은 이라는 뜻이 있어요.").queue();
            channel.sendMessage("\n운세 풀이 : 오늘은 당신이 한 노력에 마지막에 달했어요. 일이 순조롭게 진행되고 있네요. 이 일을 끝내고 새로운 것을 시작 하려고 할 것 같아요. 또는 멀리 여행을 가는것도 나쁘지 않아 보여요." +
    ...
    private String getisCard(long userId){
    ...
    private void updateToday(String card,String date, long userId) {
        try (final PreparedStatement preparedStatement = MySQLDatabase
            ...
            preparedStatement.setString(1, String.valueOf(card + "-" + date));
    ...
    public String getHelp() {
        return "You can use tarot cards to find out your fortune for the day. The cards are based on the Rider Weight deck and use only Major Arcana.";
    }
}
